...
interface OvertimeApprovalDetailModalProps {
...
export function OvertimeApprovalDetailModal({
  request,
  open,
  onOpenChange,
  onApprove,
  onReject,
}: OvertimeApprovalDetailModalProps) {
  const { translations, language } = useLanguage()

  // Format date and time
  const formatDateTime = (dateTimeString: string) => {
    const date = new Date(dateTimeString)
    return format(date, "PPP p", { locale: language === "tr" ? trLocale : undefined })
  }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex items-center">
                    <Calendar className="h-4 w-4 mr-2 text-muted-foreground" />
                    <span>{formatDateTime(request.endDateTime)}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center">
                    ...
                    <span>
                      Submitted on{" "}
                      {format(new Date(request.createdAt), "PPP", { locale: language === "tr" ? trLocale : undefined })}{" "}
                      at{" "}
